...
const initialOrders = [
  {
    ...
    products: [
      { productId: 'P001', name: 'Product A', price: 29.99, quantity: 2, imageUrl: 'https://via.placeholder.com/50' },
      { productId: 'P002', name: 'Product B', price: 49.99, quantity: 1, imageUrl: 'https://via.placeholder.com/50' },
    ],
  ...
  },
  {
    ...
    products: [
      { productId: 'P003', name: 'Product C', price: 19.99, quantity: 3, imageUrl: 'https://via.placeholder.com/50' },
    ],
...
export default function Order_List() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-300 min-h-screen">
      {/* Title */}
      <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-6 text-gray-800">Order List</h1>
        ...
        <table className="min-w-full bg-white border border-gray-200">
          <thead className="bg-gray-200 text-gray-700">
            <tr>
              ...
              <th className="px-4 py-2 text-left text-xs font-semibold uppercase">Client Name</th>
              <th className="px-4 py-2 text-left text-xs font-semibold uppercase">Address</th>
              ...
              <th className="px-4 py-2 text-left text-xs font-semibold uppercase">Products</th>
              <th className="px-4 py-2 text-left text-xs font-semibold uppercase">Tracking Number</th>
            ...
            {filteredOrders.map(order => (
              <tr key={order.id} className="hover:bg-gray-50 border-b border-gray-200">
                ...
                <td className="px-4 py-2 text-sm">
                  <span className={`font-semibold ${order.status === 'Approved' ? 'text-green-600' : 'text-yellow-600'}`}>
                    {order.status}
                  </span>
                </td>
                <td className="px-4 py-2 text-sm">
                  {order.products.map((product, index) => (
                    <div key={index} className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm">{product.name} (Price: ${product.price}, Quantity: {product.quantity})</span>
                    </div>
                  ))}
                </td>
